...
Connector actions are custom capabilities that extend C1 automations with app-specific operations. Both Snowflake actions are reached from the **Account lifecycle action** automation step. Which step offers an action is determined by the action itself — you cannot choose.
...
Neither action appears in the [Perform connector action](/product/admin/automations-steps-reference#perform-connector-action) step, which lists actions scoped to a resource type.
...
When you invoke either action directly through the CLI, pass the Snowflake username as `user_id`. Repeating an action on a user already in the target state succeeds and changes nothing else.
