...
|-------------|-------------------|-------------|
| `enable_user` | `user_id` (string, required) | Reactivates a Snowflake user |
| `disable_user` | `user_id` (string, required) | Deactivates a Snowflake user |

<Note>
**`enable_user`/`disable_user` require `OWNERSHIP` on the target user.** These actions run as the `USERADMIN` role. If the service account's `USERADMIN` role doesn't own the target user (for example, a user created by `ACCOUNTADMIN` or another role), the action fails with a privilege error. Grant `USERADMIN` ownership of the users it needs to manage, or have it inherit `MANAGE GRANTS`.
