...
type Snowflake struct {
	...
	SyncTables bool `mapstructure:"sync-tables"`
...
func (c *Snowflake) GetStringSlice(fieldName string) []string {
...
func (c *Snowflake) GetString(fieldName string) string {
	v, ok := c.findFieldByTag(fieldName)
	if !ok {
		return ""
	}
...
func (c *Snowflake) GetInt(fieldName string) int {
	v, ok := c.findFieldByTag(fieldName)
	if !ok {
		return 0
	}
...
func (c *Snowflake) GetBool(fieldName string) bool {
	v, ok := c.findFieldByTag(fieldName)
	if !ok {
		return false
	}
...
func (c *Snowflake) GetStringMap(fieldName string) map[string]any {
	v, ok := c.findFieldByTag(fieldName)
	if !ok {
